...
export interface ScannedOrgData {
  name?: string;
  pan?: string;
  gst?: string;
  taxReg?: string;
...
interface ScanField { key: keyof ScannedOrgData; label: string; required: boolean; }

const FIELDS: ScanField[] = [
  { key: "name",    label: "Organization Name", required: true },
  { key: "pan",     label: "PAN",               required: true },
  { key: "gst",     label: "GSTIN",             required: true },
  { key: "email",   label: "Email",             required: true },
  { key: "phone",   label: "Phone",             required: false },
  { key: "taxReg",  label: "Tax Registration",  required: false },
...
function simulateScan(fileName: string): ScannedOrgData {
  ...
  if (isPdf) {
    return {
      ...
      address: "No. 8, IT Expressway, Sholinganallur",
      city: "Chennai",
      state: "Tamil Nadu",
      pincode: "600119",
      country: "India",
...
interface Props { onApply: (d: ScannedOrgData) => void; }

export function AIOrganizationScanner({ onApply }: Props) {
  const [state, setState] = useState<ScanState>("idle");
  const [fileName, setFileName] = useState("");
  const [fileSize, setFileSize] = useState("");
  const [result, setResult] = useState<ScannedOrgData | null>(null);
  ...
  if (state === "idle") return (
    <div className="rounded-xl flex flex-col gap-3 p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: "var(--muted-foreground)" }}>Upload a registration certificate, GST document, or letterhead to extract organization details</p>
          </div>
        </div>
      ...
      <div
        ...
        onDrop={e => { e.preventDefault(); e.currentTarget.style.borderColor = "var(--border)"; const f = e.dataTransfer.files?.[0]; if (f) startScan(f); }}
      >
    ...
    <div className="rounded-xl p-5 flex flex-col gap-4" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p style={{ fontSize: 11, color: "var(--muted-foreground)" }}>Extracting organization details from {fileName}</p>
        </div>
      </div>
      <div className="rounded-full overflow-hidden" style={{ height: 4, background: "var(--secondary)" }}>
        <div style={{ height: "100%", width: `${progress}%`, background: "var(--foreground)", borderRadius: 9999, transition: "width 0.18s ease" }} />
      </div>
      <div className="flex flex-wrap gap-2">
        {["Reading document", "Extracting org name", "Identifying tax numbers", "Parsing address"].map((step, i) => (
    ...
    <div className="rounded-xl flex flex-col gap-0 overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      ...
      <div className="px-5 py-4 flex flex-col gap-3">
        ...
        <div className="grid gap-2" style={{ gridTemplateColumns: "repeat(3, 1fr)" }}>
          {found.map(f => (
            <div key={f.key} className="flex items-start gap-2 rounded-lg px-3 py-2"
      ...
      <div className="flex items-center justify-between px-5 py-3" style={{ borderTop: "1px solid var(--border)", background: "var(--secondary)" }}>
        <p style={{ fontSize: 12, color: "var(--muted-foreground)" }}>
          {allRequiredFound ? "Ready to apply. Review the form below before creating." : "Apply what was found, then fill in the missing fields."}
        </p>
